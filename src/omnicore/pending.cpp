 ...
 */
void PendingAdd(const uint256& txid, const std::string& sendingAddress, uint16_t type, uint32_t propertyId, int64_t amount)
{
    if (msc_debug_pending) PrintToLog("%s(%s,%s,%d,%d,%d)\n", __func__, txid.GetHex(), sendingAddress, type, propertyId, amount);
    ...
    if (type != MSC_TYPE_METADEX_CANCEL_PRICE && type != MSC_TYPE_METADEX_CANCEL_PAIR && type != MSC_TYPE_METADEX_CANCEL_ECOSYSTEM) {
        if (!update_tally_map(sendingAddress, propertyId, -amount, PENDING)) {
            PrintToLog("ERROR - Update tally for pending failed! %s(%s,%s,%d,%d,%d)\n", __func__, txid.GetHex(), sendingAddress, type, propertyId, amount);
            return;
        }
    }
