                          ...
                          int64_t referenceAmount, const std::vector<unsigned char>& data, uint256& txid, std::string& rawHex, bool commit)
{
    ...
    for (std::vector<std::pair<CScript, int64_t> >::iterator it = vecSend.begin(); it != vecSend.end(); ++it) {
        CRecipient recipient = {it->first, it->second, false}; // TODO: CHECK
        vecRecipients.push_back(recipient);
    }
    int nChangePosRet;
    if (!pwalletMain->CreateTransaction(vecRecipients, wtxNew, reserveKey, nFeeRet, nChangePosRet, strFailReason, &coinControl)) { return MP_ERR_CREATE_TX; }
...
void CMPSTOList::getRecipients(const uint256 txid, string filterAddress, UniValue *recipientArray, uint64_t *total, uint64_t *stoFee)
{
  if (!pdb) return;
...
void CMPTradeList::getTradesForPair(uint32_t propertyIdSideA, uint32_t propertyIdSideB, UniValue& responseArray, uint64_t count)
{
  ...
  std::reverse(responseArray.getValues().begin(), responseArray.getValues().end()); // TODO: CHECK
  delete it;
}
