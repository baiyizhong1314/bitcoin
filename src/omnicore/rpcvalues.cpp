...
std::vector<PrevTxsEntry> ParsePrevTxs(const UniValue& value)
{
    ...
    BOOST_FOREACH(const UniValue& p, prevTxs.getValues()) {
        if (!p.isObject()) {
            throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "expected object with {\"txid\",\"vout\",\"scriptPubKey\",\"value\":n.nnnnnnnn}");
        }
        UniValue prevOut = p.get_obj();
        // TODO: CHECK
        //RPCTypeCheck(prevOut, boost::assign::map_list_of("txid", UniValue::str_type)("vout", UniValue::int_type)("scriptPubKey", UniValue::str_type)("value", UniValue::real_type));
