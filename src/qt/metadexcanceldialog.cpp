...
void MetaDExCancelDialog::SendCancelTransaction()
{
    ...
    if (result != 0) {
        ...
        if (!autoCommit) {
            ...
            if (action == 2) PendingAdd(txid, fromAddress, MSC_TYPE_METADEX_CANCEL_PRICE, propertyIdForSale, amountForSale);
            if (action == 3) PendingAdd(txid, fromAddress, MSC_TYPE_METADEX_CANCEL_PAIR, propertyIdForSale, 0);
            if (action == 4) PendingAdd(txid, fromAddress, MSC_TYPE_METADEX_CANCEL_ECOSYSTEM, propertyIdForSale, 0);
